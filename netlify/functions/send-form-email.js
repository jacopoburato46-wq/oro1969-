...
function buildPrintHtml(form = {}, items = []) {
...
exports.handler = async (event) => {
  try {
    ...
    const printHtml = buildPrintHtml(form, items);
    const pdfBuffer = await generatePdfBuffer(printHtml);
    ...
    const response = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        text: `In allegato trovi il PDF della scheda rientro oro #${form.scheda_number || 'Senza numero'}.`,
        attachments: [
          {
            filename: `scheda-rientro-oro-${form.scheda_number || 'senza-numero'}.pdf`,
            content: pdfBase64
          }
        ]
      })
